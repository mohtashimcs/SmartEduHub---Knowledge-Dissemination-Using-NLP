      ...
      <div className="flex items-center py-3 space-x-2">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            {table
              .getAllColumns()
              .filter((column) => column.getCanHide() && column.id !== "uid") // Filter out the "uid" column
              .map((column) => {
                return (
                  <DropdownMenuCheckboxItem
                    key={column.id}
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            {data &&
              data.length > 0 &&
              Array.from(new Set(data.map((item) => item.userRole)))
                ...
                .map((role) => {
                  ...
                  return (
                    <DropdownMenuItem
                      key={role}
                      onClick={() => {
                        table.getColumn("userRole")?.setFilterValue(role);
                      }}
                    >
                      {capitalizedRole}
                    </DropdownMenuItem>
            ...
            <DropdownMenuItem
              onClick={() => {
                table.getColumn("userRole")?.setFilterValue("");
              }}
            >
              Show All
            </DropdownMenuItem>
